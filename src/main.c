...
static void main_window_load(Window *window) {
  // Create GFont
  s_time_font = fonts_load_custom_font(resource_get_handle(RESOURCE_ID_FONT_CLOCK_PROVA_65));
  s_date_font = fonts_load_custom_font(resource_get_handle(RESOURCE_ID_FONT_CLOCK_20));
  s_weather_font = fonts_load_custom_font(resource_get_handle(RESOURCE_ID_FONT_CLOCK_PROVA_30));

  // Create time TextLayer
  s_time_layer = text_layer_create(GRect(0, 20, 144, 70));
  ...
  s_weather_layer = text_layer_create(GRect(75, 110, 69, 50));
  text_layer_set_background_color(s_weather_layer, GColorClear);
  text_layer_set_text_color(s_weather_layer, GColorWhite);
  text_layer_set_text(s_weather_layer, "Load");
  
  // Create GBitmap, then set to created BitmapLayer
  s_background_bitmap = gbitmap_create_with_resource(RESOURCE_ID_IMAGE_ERROR);
  s_background_layer = bitmap_layer_create(GRect(10, 98, 60, 60));
...
static void inbox_received_callback(DictionaryIterator *iterator, void *context) {
  ...
  if (condition_code == 30){
      ...
      bitmap_layer_set_bitmap(s_background_layer, s_background_bitmap);
  } else if (condition_code >= 26 && condition_code <= 28) {
      ...
      bitmap_layer_set_bitmap(s_background_layer, s_background_bitmap);
  } else if (condition_code == 32 || condition_code == 34) {
      s_background_bitmap = gbitmap_create_with_resource(RESOURCE_ID_IMAGE_CLEAR_DAY);
      bitmap_layer_set_bitmap(s_background_layer, s_background_bitmap);
  } else if ((condition_code >= 41 && condition_code <= 43) || condition_code == 16) {
